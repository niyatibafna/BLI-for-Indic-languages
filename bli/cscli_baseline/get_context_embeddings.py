...
def get_lm(tokenizer_path: str, model_path: str) -> tuple:
    ...
    tokenizer = get_tokenizers.train_or_load_tokenizer(tokenizer_path)
...
def get_word_embeddings_from_model(tokenizer, model, sentences: list, word_indices: list) -> np.array:
    ...
    for sentence_index, word_index in word_indices:
        ...
        if word_subword_indices is None: # The word is not in the sentence due to truncation
            yield None
            continue
...
def build_contextual_embeddings_matrix(filename: str, tokenizer, model, output_path, \
                                       top_words: list, min_contexts: int = 2, max_contexts: int = 10, \
                                          batch_size = 32) -> np.array:
    ...
    for sentence in read_data(filename):
        ...
        if len(context_matrix) % 1000 == 0:
            ...
            save_embeddings(context_matrix, output_path, min_contexts)
...
def save_embeddings(context_matrix: dict, output_path: str, min_contexts: int = 15):
    ...
    context_matrix = {word: context_matrix[word][0] / context_matrix[word][1] \
                      ...
                      and context_matrix[word][1] >= min_contexts}
...
def main(args):
    ...
    context_matrix = build_contextual_embeddings_matrix(args.corpus_path, tokenizer, model, args.output_path, \
                                                        top_words, min_contexts=args.min_contexts, max_contexts = args.max_contexts, \
                                                            batch_size = args.batch_size)

    save_embeddings(context_matrix, args.output_path, args.min_contexts)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_words', type=int, default=50000, help='Number of words to get embeddings for')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--min_contexts', type=int, default=2, help='Minimum number of contexts to get embeddings for a word')
    parser.add_argument('--max_contexts', type=int, default=10, help='Maximum number of contexts to get embeddings for a word')
